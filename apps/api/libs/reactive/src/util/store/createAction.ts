...
import { createAction as createActionOrigin } from '@reduxjs/toolkit';
...
export const createGeneralActionType = (type: string, prefix?: string) => ({
  ...
  AFTER: prefix ? `+${prefix}/${type}_AFTER_ACTION` : `${type}_AFTER_ACTION`,
  ERROR: prefix ? `+${prefix}/${type}_ERROR_ACTION` : `${type}_ERROR_ACTION`,
});
